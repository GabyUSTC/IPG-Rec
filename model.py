...
class SASRec(torch.nn.Module):
    ...
    def log2feats(self, log_seqs):
        ...
        # attention_mask = ~torch.tril(torch.ones((tl, tl), dtype=torch.bool, device=self.dev))
        # attention_mask.float().masked_fill(attention_mask == 0, float('-inf')).masked_fill(attention_mask == 1, float(0.0))
        # attention_mask[:, -1] = torch.zeros(tl).bool()
    ...
    def next_item_prediction(self, log_seqs, interacted, target_item, rec2inf=False, k=50):
        log_feats = self.log2feats(log_seqs)
        final_feat = log_feats[:, -1, :]
        item_embs = self.item_emb(torch.LongTensor(range(1, self.item_num + 1)).to(self.dev))
        if not rec2inf:
            item_similarity = torch.matmul(item_embs[target_item], item_embs.T)
            logits = torch.matmul(final_feat, item_embs.T) + self.alpha * item_similarity
